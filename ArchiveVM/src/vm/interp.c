/* Automatically generated from Squeak on 30 March 2011 9:23:03 am 
   by VMMaker 3.11.13
 */
...
#define HandlePropagationSelectorIndex 57
...
sqInt internalIsHandle(sqInt oop);
#pragma export on
...
sqInt isActiveMetaHandle(sqInt oop);
...
sqInt isSubHandle(sqInt aClass);
...
#pragma export on
EXPORT(sqInt) primitiveAsHandle(void);
#pragma export off
...
sqInt extraVMMemory;
const char *interpreterVersion = "PharoCore1.1ALPHA of '19 October 2009' [Latest update: #11239]";
...
sqInt classAlien(void) {
	return longAt((foo->specialObjectsOop + BaseHeaderSize) + (ClassAlien << ShiftForWord));
}
...
sqInt classUnsafeAlien(void) {
	return longAt((foo->specialObjectsOop + BaseHeaderSize) + (ClassUnsafeAlien << ShiftForWord));
}
		...
		case 0:
			/* pushReceiverVariableBytecode */
			{
				...
				if (internalIsHandle(foo->receiver)) {
					...
					/* begin internalPush: */
					longAtPointerput(localSP += BytesPerWord, longAt((foo->receiver + BaseHeaderSize) + (fieldIndex << ShiftForWord)));
				}
			}
;
			break;
		...
		case 96:
			/* storeAndPopReceiverVariableBytecode */
			{
				...
				if (internalIsHandle(rcvr)) {
					/* begin storeReceiverVariableInHandle:ofObject:value: */
					fieldIndex = 96 & 7;
					if ((((usqInt) (longAt((rcvr + BaseHeaderSize) + (HandleConfigurationIndex << ShiftForWord)))) >> 1) & 1) {
						...
						stObjectatput(stateArray, fieldIndex + 1, top);
					} else {
						/* begin storePointer:ofObject:withValue: */
						if ((((usqInt) (longAt((rcvr + BaseHeaderSize) + (HandleReceiverIndex << ShiftForWord))))) < (((usqInt) foo->youngStart))) {
							possibleRootStoreIntovalue(longAt((rcvr + BaseHeaderSize) + (HandleReceiverIndex << ShiftForWord)), top);
						}
						longAtput(((longAt((rcvr + BaseHeaderSize) + (HandleReceiverIndex << ShiftForWord))) + BaseHeaderSize) + (fieldIndex << ShiftForWord), top);
					}
				} else {
					if ((((usqInt) rcvr)) < (((usqInt) foo->youngStart))) {
...
;
		...
		case 112:
			/* pushReceiverBytecode */
			{
				/* begin fetchNextBytecode */
				currentBytecode = byteAtPointer(++localIP);
				if ((internalIsHandle(foo->receiver)) && (!((((usqInt) (longAt((foo->receiver + BaseHeaderSize) + (HandleConfigurationIndex << ShiftForWord)))) >> 1) & 4))) {
					/* begin internalPush: */
					longAtPointerput(localSP += BytesPerWord, longAt((foo->receiver + BaseHeaderSize) + (HandleReceiverIndex << ShiftForWord)));
				} else {
					/* begin internalPush: */
...
;
		...
		commonReturn:
			/* commonReturn */
			{
				...
				if ((internalIsHandle(foo->receiver)) && ((longAt((foo->receiver + BaseHeaderSize) + (HandleReceiverIndex << ShiftForWord))) == localReturnValue)) {
					localReturnValue = foo->receiver;
				}
...
;
		...
		case 129:
			/* extendedStoreBytecode */
			{
				...
				top = longAtPointer(localSP);
				if (variableType == 0) {
					if (internalIsHandle(top)) {
						top = longAt((top + BaseHeaderSize) + (HandleReceiverIndex << ShiftForWord));
					}
					if (internalIsHandle(foo->receiver)) {
						/* begin storeReceiverVariableInHandle:ofObject:value: */
						fieldIndex = currentBytecode & 7;
						aHandle = foo->receiver;
						if ((((usqInt) (longAt((aHandle + BaseHeaderSize) + (HandleConfigurationIndex << ShiftForWord)))) >> 1) & 1) {
							...
							/* begin storePointer:ofObject:withValue: */
							if ((((usqInt) (longAt((aHandle + BaseHeaderSize) + (HandleReceiverIndex << ShiftForWord))))) < (((usqInt) foo->youngStart))) {
								possibleRootStoreIntovalue(longAt((aHandle + BaseHeaderSize) + (HandleReceiverIndex << ShiftForWord)), top);
							}
							longAtput(((longAt((aHandle + BaseHeaderSize) + (HandleReceiverIndex << ShiftForWord))) + BaseHeaderSize) + (fieldIndex << ShiftForWord), top);
						}
						goto l2;
					} else {
						/* begin storePointer:ofObject:withValue: */
						...
						longAtput((oop + BaseHeaderSize) + (variableIndex << ShiftForWord), top);
						goto l2;
					}
				}
				if (variableType == 1) {
					longAtput((localHomeContext + BaseHeaderSize) + ((variableIndex + TempFrameStart) << ShiftForWord), top);
					goto l2;
				}
				if (variableType == 2) {
					error("illegal store");
				...
				if (variableType == 3) {
					...
					if ((((usqInt) association)) < (((usqInt) foo->youngStart))) {
						possibleRootStoreIntovalue(association, top);
					}
					longAtput((association + BaseHeaderSize) + (ValueIndex << ShiftForWord), top);
					goto l2;
				}
			}
;
		...
		normalSend:
			/* normalSend */
			{
				...
				if (ccIndex1 == 0) {
					...
					foo->lkupClass = longAt(((longAt((foo->specialObjectsOop + BaseHeaderSize) + (CompactClasses << ShiftForWord))) + BaseHeaderSize) + ((ccIndex1 - 1) << ShiftForWord));
					goto l45;
				}
			l45:	/* end fetchClassOf: */;
				if (internalIsHandle(rcvr)) {
					if ((((usqInt) (longAt((rcvr + BaseHeaderSize) + (HandleConfigurationIndex << ShiftForWord)))) >> 1) & 2) {
						foo->lkupClass = longAt((rcvr + BaseHeaderSize) + (HandleClassLookupIndex << ShiftForWord));
					} else {
						/* begin fetchClassOf: */
						if (((longAt((rcvr + BaseHeaderSize) + (HandleReceiverIndex << ShiftForWord))) & 1)) {
							foo->lkupClass = longAt((foo->specialObjectsOop + BaseHeaderSize) + (ClassInteger << ShiftForWord));
							goto l44;
						}
						ccIndex = (((usqInt) (longAt(longAt((rcvr + BaseHeaderSize) + (HandleReceiverIndex << ShiftForWord))))) >> 12) & 31;
						if (ccIndex == 0) {
							foo->lkupClass = (longAt((longAt((rcvr + BaseHeaderSize) + (HandleReceiverIndex << ShiftForWord))) - BaseHeaderSize)) & AllButTypeMask;
							goto l44;
						} else {
...
;
		...
		commonSend:
			...
			l46:	/* end lookupInMethodCacheSel:class: */;
				...
				longAtput(where + (ReceiverIndex << ShiftForWord), longAtPointer(localSP - (argCount2 * BytesPerWord)));
				for (i = 1; i <= argCount2; i += 1) {
...
;
		...
		case 198:
			/* bytecodePrimEquivalent */
			{
				...
				rcvr = longAtPointer(localSP - (1 * BytesPerWord));
				arg = longAtPointer(localSP - (0 * BytesPerWord));
				if (internalIsHandle(arg)) {
					arg = longAt((arg + BaseHeaderSize) + (HandleReceiverIndex << ShiftForWord));
				}
				if (internalIsHandle(rcvr)) {
					rcvr = longAt((rcvr + BaseHeaderSize) + (HandleReceiverIndex << ShiftForWord));
				}
				/* begin booleanCheat: */
				bytecode = byteAtPointer(++localIP);
				/* begin internalPop: */
...
register struct foo * foo = &fum;
	...
	cc = aClass;
	while (!(cc == foo->nilObj)) {
		if (cc == (longAt((foo->specialObjectsOop + BaseHeaderSize) + (ClassHandle << ShiftForWord)))) {
			return 1;
		}
		cc = longAt((cc + BaseHeaderSize) + (SuperclassIndex << ShiftForWord));
	}
	return 0;
}
...
register struct foo * foo = &fum;
	...
	if (isSubHandle(fetchClassOf(rcvr))) {
		if (isActiveMetaHandle(rcvr)) {
			/* begin primitiveFail */
			foo->successFlag = 0;
			return null;
		} else {
			bitValue = longAt(rcvr);
			bitValue = bitValue | HandleBit;
			longAtput(rcvr, bitValue);
		}
	}
}
...
register struct foo * foo = &fum;
	...
	foo->stackPointer -= BytesPerWord;
	thisObject = top1;
	if (internalIsHandle(otherObject)) {
		otherObject = longAt((otherObject + BaseHeaderSize) + (HandleReceiverIndex << ShiftForWord));
	}
	/* begin pushBool: */
...
register struct foo * foo = &fum;
	...
	if ((fetchClassOf(callbackContext)) != (longAt((foo->specialObjectsOop + BaseHeaderSize) + (ClassMethodContext << ShiftForWord)))) {
		return 0;
	}
	...
	foo->stackPointer = (calloutContext + BaseHeaderSize) + (((TempFrameStart + tmp) - 1) * BytesPerWord);
	return 1;
}
...
register struct foo * foo = &fum;
	...
	foo->receiver = longAt((foo->specialObjectsOop + BaseHeaderSize) + (ClassAlien << ShiftForWord));
	/* begin fetchClassOfNonInt: */
	ccIndex = (((usqInt) (longAt(foo->receiver))) >> 12) & 31;
	if (ccIndex == 0) {
		foo->lkupClass = (longAt(foo->receiver - BaseHeaderSize)) & AllButTypeMask;
		goto l1;
	} else {
		foo->lkupClass = longAt(((longAt((foo->specialObjectsOop + BaseHeaderSize) + (CompactClasses << ShiftForWord))) + BaseHeaderSize) + ((ccIndex - 1) << ShiftForWord));
		goto l1;
...
l1:	/* end fetchClassOfNonInt: */;
	foo->messageSelector = longAt((foo->specialObjectsOop + BaseHeaderSize) + (InvokeCallbackSelector << ShiftForWord));
	...
	longAtput(where1 + (SenderIndex << ShiftForWord), foo->activeContext);
	...
	longAtput(where1 + (MethodIndex << ShiftForWord), foo->newMethod);
	longAtput(where1 + (ReceiverIndex << ShiftForWord), foo->receiver);
	...
	foo->reclaimableContextCount += 1;
	foo->activeContext = newContext;
	where = (foo->activeContext + BaseHeaderSize) + (ReceiverIndex << ShiftForWord);
